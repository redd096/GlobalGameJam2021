...
public class GridAStarEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        {
            ((GridAStar)target).SetGridSize();
            ((GridAStar)target).CreateGrid();
...
[AddComponentMenu("redd096/PathFinding AStar/Grid A Star")]
    ...
    void OnDrawGizmosSelected()
    {
        ...
        if (grid != null)
        {
            foreach (Node node in grid)
            {
                ...
                Gizmos.color = node.isWalkable ? Color.blue : Color.red;
    ...
    public void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSize.x; x++)
        {
            for(int y = 0; y < gridSize.y; y++)
            {
                ...
                bool isWalkable = is3D ?
                    !Physics.CheckSphere(worldPosition, nodeRadius, unwalkableMask) :       //use 3d
                    !Physics2D.OverlapCircle(worldPosition, nodeRadius, unwalkableMask);    //or 2
